...
for i=1:length(maturities)
    whole_dates=floor(yearfrac(start,maturities(i))/tau);
    sum=0;
    ts=1:whole_dates;
    ts=ts*tau;
    for j=1:length(ts)
    ...
    sum=sum+res_tau*swaprate*DiscFactor(zeroCurve,yearfrac(start,maturities(i)),compounding);
    
    
    RFS(i)=Notional*(-DiscFactor(zeroCurve,start,compounding)+DiscFactor(zeroCurve,yearfrac(start,maturities(i)),compounding)+sum);
end
